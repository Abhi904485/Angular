  ...
  .controller('mycontroller', function($scope) {
    ...
    $scope.search = function(item) {
      if ($scope.search_filter == undefined) {
        ...
        if (item.name.toLowerCase().indexOf($scope.search_filter.toLowerCase()) != -1 ||
        item.salary.toLowerCase().indexOf($scope.search_filter.toLowerCase()) != -1) {
          return true;
        }
      }
      return false;
